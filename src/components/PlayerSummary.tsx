    ...
    <>
      <div className={styles.playerHead}>
        ...
        {data.response.banned ? (
          ...
          data.response.muted && <span className={styles.bannedOrMuted}>MUTADO</span>
        )}
      </div>
      ...
      <dl className={styles.playerInfo}>
        <div className={styles.wrapper}>
          ...
          <dd>
            {data.response.connected ? (
              <span style={{ color: '#0db91b', fontWeight: '600' }}>Online</span>
            ) : (
        ...
        <div className={styles.wrapper}>
          <dt>Tipo de conta</dt>
          <dd>{data.response.account.type == 'premium' ? 'Original' : 'Pirata'}</dd>
        </div>
        ...
        <div className={styles.wrapper}>
          ...
          <dd>
            {data.response.clan ? (
              <span className={styles.shadowedBold} style={{ color: `${data.response.clan.tag_color}` }}>
                {data.response.clan.tag}
              </span>
            ) : (
              '-'
        ...
        <div className={styles.wrapper}>
          <dt>Melhor tag</dt>
          <dd>
            <span className={styles.shadowedBold} style={{ color: `${data.response.best_tag.color}` }}>
            ...
            {data.response.best_tag.data && (
              <span
                ...
                style={{
                  color: data.response.best_tag.data?.plus.replace('_', ''),
                }}
              >
